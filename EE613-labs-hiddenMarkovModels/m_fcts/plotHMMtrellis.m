function plotHMMtrellis(Trans, StatesPriors, gridTrans, gridNode, colTint)
...
for i=1:nbStates
	...
	plot2DArrow(posTmp, dirTmp, [.8 .8 .8]); 
end
	...
	for i=1:nbStates
		for j=1:nbStates
			...
			if gridTrans(i,j,t)==0
				...
				plot2DArrow(posTmp,dirTmp,cTmp,4);
			end
		end
	end
end 
...
for t=1:nbData
	for i=1:nbStates
		...
		if gridNode(i,t)==0
			patch(meshTmp(1,:), meshTmp(2,:), [.8 .8 .8],'edgecolor',[.6 .6 .6],'linewidth',2,'facealpha',valAlpha,'edgealpha',valAlpha);
		else
			cTmp = colTint(gridNode(i,t),:);
			patch(meshTmp(1,:), meshTmp(2,:), min(cTmp+.5,1),'edgecolor',cTmp,'linewidth',2,'facealpha',valAlpha,'edgealpha',valAlpha);
		end
		text(nodePos(1,i,t),nodePos(2,i,t),['$s_' num2str(i) '$'],'interpreter','latex','fontsize',22, ...
			'HorizontalAlignment','center','VerticalAlignment','middle'); %,'FontWeight','bold'
	end
	text(nodePos(1,end,t),nodePos(2,end,t)-nodeRadius*2,['$t\!=\!' num2str(t) '$'],'interpreter','latex','fontsize',22, ...
